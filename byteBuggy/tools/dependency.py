...
class Dependency(object):
    ...
    def run_dependency_check(cls):
        ...
        if missing_required:
            print('At least 1 Required app is missing. byteBuggy needs Required apps to run')
            import sys
            sys.exit(-1)
    ...
    def fails_dependency_check(cls):
        ...
        if cls.dependency_required:
            print('Error: Required app %s was not found' % cls.dependency_name)
            print('. install @ %s' % cls.dependency_url)
            return True

        else:
            print('Warning: Recommended app %s was not found' % cls.dependency_name)
            print('. install @ %s' % cls.dependency_url)
            return False
