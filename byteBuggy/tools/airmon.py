...
class Airmon(Dependency):
    ...
    @staticmethod
    def ask(max_attempts=5):
        '''
        Asks user to define which wireless interface to use, with retry logic.
        Does not ask if:
            1. There is already an interface in monitor mode, or
            2. There is only one wireless interface (automatically selected).
        Puts selected device into Monitor Mode after successful retry, if initially failed.
    
        Args:
           max_attempts (int): Maximum number of attempts to try enabling monitor mode.
        Returns:
            The interface successfully put into monitor mode.
        Raises:
            Exception: If unable to find wireless interfaces or put any into monitor mode after retries.
        '''

        Airmon.terminate_conflicting_processes()

        for attempt in range(1, max_attempts + 1):
            print(f'\nAttempt {attempt} of {max_attempts} to enable monitor mode...')

            print('\nLooking for wireless interfaces...')
            monitor_interfaces = Iwconfig.get_interfaces(mode='Monitor')
            if len(monitor_interfaces) == 1:
                # Using the device already in monitor mode
                iface = monitor_interfaces[0]
                print(f'Using {iface} already in monitor mode')
            ...
            if count == 0:
                if attempt < max_attempts:
                    print('No wireless interfaces found, retrying...')
                    continue
                else:
                    # Final attempt, no interfaces found
                    raise Exception('airmon-ng did not find any wireless interfaces after several attempts')
            ...
            if count == 1:
                ...
                question = f'Select wireless interface (1-{count}): '
                choice = input(question)

            iface = a.get(choice)

            if iface.interface in monitor_interfaces:
                print(f'{iface.interface} is already in monitor mode')
                return iface.interface
            else:
                try:
                    iface.interface = Airmon.start(iface)
                    print(f'{iface.interface} successfully put into monitor mode')
                    return iface.interface
                except Exception as e:
                    print(f'Failed to put {iface} into monitor mode: {e}')
                    if attempt >= max_attempts:
                        raise Exception(f'Failed to put {iface} into monitor mode after {max_attempts} attempts')
