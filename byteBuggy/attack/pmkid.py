...
class AttackPMKID(Attack):
    ...
    def run(self):
        ...
        if len(missing_deps) > 0:
            print(' Skipping PMKID attack, missing required tools: %s' % ', '.join(missing_deps))
            return False
        ...
        try:
            ...
            print('\n Failed to crack PMKID: Cracking interrupted by user')
            self.success = False
            return False
    ...
    def capture_pmkid(self):
        ...
        while self.timer.remaining() > 0 and self.keep_capturing:
            pmkid_hash = pcaptool.get_pmkid_hash(self.pcapng_file)
            if pmkid_hash is not None:
                break  # Got PMKID

            Color.pattack('PMKID', self.target, 'CAPTURE',
                    'Waiting for PMKID (%s)' % str(self.timer))
            time.sleep(1)
        ...
        if pmkid_hash is None:
            Color.pattack('PMKID', self.target, 'CAPTURE',
                    'Failed to capture PMKID\n')
            print('')
            return None  # No hash found.

        Color.clear_entire_line()
        Color.pattack('PMKID', self.target, 'CAPTURE', 'Captured PMKID')
    ...
    def crack_pmkid_file(self, pmkid_file):
        ...
        if Configuration.wordlist is None:
            print('\n Not cracking PMKID ' +
                    'because there is no wordlist (re-run with --dict)')

            # TODO: Uncomment once --crack is updated to support recracking PMKIDs.
            #print(' Run byteBuggy with the --crack and --dict options to try again.')

            key = None
        else:
            Color.clear_entire_line()
            Color.pattack('PMKID', self.target, 'CRACK', 'Cracking PMKID using %s ...\n' % Configuration.wordlist)
        ...
        if key is None:
            ...
            if Configuration.wordlist is not None:
                ...
                Color.pattack('PMKID', self.target, 'CRACK',
                        'Failed Passphrase not found in dictionary.\n')
            return False
        else:
            # Successfully cracked.
            Color.clear_entire_line()
            Color.pattack('PMKID', self.target, 'CRACKED', 'Key: %s' % key)
    ...
    def save_pmkid(self, pmkid_hash):
        ...
        print('\n Saving copy of PMKID Hash to %s ' % pmkid_file)
